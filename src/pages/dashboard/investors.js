...
import { InvestorsSearch } from 'src/sections/investors/investors-search';
import { InvestorsTable } from 'src/sections/investors/investors-table';
import { useTheme } from '@mui/material/styles';
import Paper from '@mui/material/Paper';
import { AccountProfileDetails } from 'src/sections/account/account-profile-details';
import { AddInvestors } from 'src/sections/investors/investors-add';
import { useDispatch, useSelector } from 'react-redux';
import { addWithdrawl, getAllInvestors } from 'src/redux/Actions/AdminActions';
...
const Page = () => {
  ...
  const [investorsIds, setInvestorsIds] = useState();
  const investorsSelection = useSelection(investorsIds);
  ...
  const handleWithdraw = (val) => {
    dispatch(addWithdrawl(val.email, val.withdrawn));
    setOpen(false);
  }
  ...
  useEffect(() => {
    ...
    if(withdrawlMessage){
      toast.success(withdrawlMessage, toastOptions);
      dispatch({type: "clearMessage"});
    }
    if(withdrawlError){
        ...
        <Container maxWidth="xl">
          <Stack spacing={3}>
            ...
            <InvestorsTable
              ...
              count={count}
              items={customers}
              onDeselectAll={investorsSelection.handleDeselectAll}
              onDeselectOne={investorsSelection.handleDeselectOne}
              onPageChange={handlePageChange}
              onRowsPerPageChange={handleRowsPerPageChange}
              onSelectAll={investorsSelection.handleSelectAll}
              onSelectOne={investorsSelection.handleSelectOne}
              page={page}
              rowsPerPage={rowsPerPage}
              selected={investorsSelection.selected}
            />}
          </Stack>
        </Container>
      </Box>}
